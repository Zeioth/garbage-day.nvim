...
| `stop_invisible` | `false` | Set it to `true` to stop all LSP clients except the ones used by the visible buffers. This option triggers every time you change to a different buffer and ignores the grace period. This ensures the minimum RAM consumption. NOTE: If your CPU is not very powerful, it might take a few seconds to bring back LSP, so disable this option if you need. |
